...
@dp.message_handler(
    ...
    == "админами не становятся - админами рождаются!"
...
async def Russia_become_the_admin(message: types.Message):
    ...
    if gr == []:
        ...
        gr1 = sorted(
            [
                int(x[0].split("_")[1])
                for x in c_u.execute("select groups from admins").fetchall()
            ]
        )
        gr = "group_" + str(gr1[-1] + 1)
    if (
        c_u.execute(
            "select * from admins where tgID=?", (message.from_user.id,)
        ).fetchone()
        is None
    ):
        c_u.execute(
            'insert into admins (groups, tgID) values ("{}", {})'.format(
        ...
        if (
            c_u.execute(
                "select * from parents where tgID=? and nickname=? and username=?",
                (
        ...
        ready = InlineKeyboardButton(
            text="Готов(а)✅", callback_data="Ready_for_register"
        )
        ...
        await message.answer(
            "О Великий админ, как бы ни было прискорбно, но вы не можете стать админом дважды!"
        )
...
async def send_welcome(message: types.Message):
    ...
    for chat_member in message.new_chat_members:
        if chat_member.id == bot_id:
            ...
            for us in _users:
                exist_parent = c_u.execute(
                    "select * from parents where tgID=?", (us[0],)
                ).fetchone()

                if exist_parent is not None:
                    if (
                        c_u.execute(
                            "select * from parents where groups=? and tgID=? and groupID=?",
                            (gr, us[0], message.chat.id),
                        ).fetchone()
                        is None
                    ):
                        try:
                            if (
                                ...
                                c_u.execute(
                                    "update parents set groupID=? where groups=? and tgID=?",
                                    (
                                        message.chat.id,
                                        gr,
                                        us[0],
                                ...
                                if "True" in [
                                    ...
                                    no_org = set(
                                        c_u.execute(
                                            "select groups from parents where groupID=?",
                                            (message.chat.id,),
                                        ).fetchall()
                                    )
                                    for nrg in no_org:
                                    ...
                                    grr = c_u.execute(
                                        "select groups from admins where tgID=?",
                                        (us[0],),
                                    ).fetchone()[0]
                                    q2 = c_u.execute(
                                        "insert into parents (groups, tgID, last_name, first_name, groupID, organisator, nickname, username) "
                    ...
                    if (
                        c_u.execute(
                            "select * from parents where groups=? and tgID=? and username=? and nickname=? and groupID=?",
                            (gr, us[0], us[2], us[1], message.chat.id),
                        ).fetchone()
                        is None
                    ):
                        q2 = c_u.execute(
            ...
            _users = await fix_file.vecher_v_hatu(message.chat.id)
            gr2 = c_u.execute(
                "select groups from groups where groupID=?", (message.chat.id,)
            ).fetchall()
            for grps in gr2:
                for us in _users:
                    exist_parent = c_u.execute(
                    ...
                    if exist_parent is not None:
                        if (
                            c_u.execute(
                                "select * from parents where groups=? and tgID=? and username=? and nickname=? and groupID=?",
                                (grps[0], us[0], us[2], us[1], message.chat.id),
                            ).fetchone()
                            is None
                        ):
                            if "True" in [
                                ...
                                grr = c_u.execute(
                                    "select groups from admins where tgID=?", (us[0],)
                                ).fetchone()[0]
                                q2 = c_u.execute(
                                    "insert into parents (groups, tgID, last_name, first_name, groupID, organisator, nickname, username) "
                                    'values ("{}", {}, "{}", "{}", {}, "{}", "{}", "{}")'.format(
...
@dp.message_handler(
    lambda message: message.text.lower() == "/register"
    and message.chat.type == "private"
)
...
async def ready_for_register(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(
        chat_id=callback_query.from_user.id, text="Пожалуйста, укажите свою Фамилию!"
    )
...
async def right_fio(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    c_u.execute(
        "UPDATE parents SET last_name=?, first_name=? WHERE tgID=?",
        (F, I, callback_query.from_user.id),
    )
    conn_u.commit()
    all_FIO = f"{F} {I}"
    c_u.execute(
        "UPDATE active_events SET parent=? WHERE tgID=?",
        (all_FIO, callback_query.from_user.id),
    )
    conn_u.commit()
    ...
    change_FIO = InlineKeyboardButton(
        text="Сменить Фамилию и/или Имя", callback_data="Ready_for_register"
    )
    ln = 0
    for g in gr:
...
async def output_organisators(callback_query: types.CallbackQuery, state: FSMContext):
    try:
        ...
        gr = set(
            c_u.execute("select groups from groups where groupID=?", (grID,)).fetchall()
        )
        orgs = []
        for i in gr:
            admins_temp = c_u.execute(
...
async def start_command(message: types.Message):
    ...
    if cht_id[0] == "-":
        ...
        await message.answer(
            "Приветствую! Теперь я смогу уведомлять тебя о новых событиях. "
...
@dp.message_handler(
    lambda message: message.text.lower() == "/add" and message.chat.type == "private"
)
async def event_name(message: types.Message, state: FSMContext):
    ...
    if adm is None:
        await message.answer(
            "У вас нет прав на выполнение данной команды. Введите, пожалуйста, секретную слово/фразу, чтобы получить доступ!"
        )
        return
    else:
        if (
            c_u.execute("select * from groups where groups=?", (adm[0],)).fetchone()
            is None
        ):
            await message.answer(
                "Вы не можете создать мероприятие, пока не добавили бота хотя бы в одну беседу!"
            )
            return
        else:
...
async def choice_rassilka(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        text=callback_query.message.text, reply_markup=keyboard.add(yeah)
    )
...
async def choice_rassilka(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    await callback_query.message.edit_text(
        callback_query.message.text, reply_markup=keyboard.add(no, nono)
    )
        ...
        if (
            ...
            for parent in prnts:
                if (
                    ...
                    namee = c_u.execute(
                        "select groupTitle from groups where groupID=?", (parent[5],)
                    ).fetchone()[0]
                    c_u.execute(
                        "insert into groups (groups, groupID, groupTitle) "
...
async def all_events_command(message: types.Message):
    ...
    if flag:
        await message.answer(
            ...
            reply_markup=kb.add(profile).add(exit),
        )
    else:
...
@dp.message_handler(
    lambda message: message.text.lower() == "/myevents"
    and message.chat.type == "private"
)
...
# Мой профиль
@dp.message_handler(
    lambda message: message.text.lower() == "/profile"
    and message.chat.type == "private"
)
async def profile(message: types.Message):
    ...
    await message.answer(
        ...
        f'❌Не оплачено мероприятий, в которых вы участвуете: {len(c_u.execute("select * from active_events where tgID=? and paid=?", (message.from_user.id, "False")).fetchall())}',
...
async def ready_for_register(callback_query: types.CallbackQuery):
    ...
    if out == "ВАШИ МЕРОПРИЯТИЯ:\n\n":
        await callback_query.message.edit_text(
            text="Вы не участвуете ни в одном из мероприятий.",
            reply_markup=kb.add(all_ev).add(bck, exit),
        )
    else:
        await callback_query.message.edit_text(
            text=out, reply_markup=kb.add(bck, all_ev).add(exit)
        )
...
async def ready_for_register(callback_query: types.CallbackQuery):
    ...
    for e in mers:
        ...
        cnt += 1
    await callback_query.message.edit_text(
        out, reply_markup=kb.add(bck, my_ev).add(exit)
    )
...
async def ready_for_register(callback_query: types.CallbackQuery):
    ...
    I = c_u.execute(
        "select first_name from parents where tgID=?", (callback_query.from_user.id,)
    ).fetchone()[0]
    gr = set(
        c_u.execute(
            "select groups from active_events where tgID=?",
...
@dp.callback_query_handler(
    ...
    in ["my_" + x[0] for x in c_u.execute("select eventID from Event").fetchall()]
)
    ...
    if (
        ...
        if pad == "True":
            await callback_query.message.edit_text(
                ...
                reply_markup=kb.add(ras, spisok)
                .add(uch, neuch)
                .add(bck, all_ev)
                .add(exit),
            )
...
async def spisok_pers(callback_query: types.CallbackQuery):
    ...
    if out == "ВАШИ МЕРОПРИЯТИЯ:\n\n":
        ...
        await callback_query.message.edit_text(
            text=out, reply_markup=kb.add(all_ev).add(bck, exit)
        )
...
@dp.callback_query_handler(
    lambda call: call.data
    in [x[0] for x in c_u.execute("select eventID from Event").fetchall()]
)
...
async def spisok_pers(callback_query: types.CallbackQuery):
    ...
    if out == f'В мероприятии "{mer}" участвуют следующие лица:\n':
        await callback_query.message.edit_text(
            ...
            reply_markup=kb.add(my_ev, all_ev).add(profile).add(exit),
        )
    else:
        await callback_query.message.edit_text(
            text=out, reply_markup=kb.add(my_ev, all_ev).add(profile).add(exit)
        )
...
@dp.message_handler(
    lambda message: "/news " in message.text.lower() and message.chat.type == "private"
)
async def profile(message: types.Message):
    try:
        gr = c_u.execute(
